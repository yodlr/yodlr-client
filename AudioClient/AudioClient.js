...
AudioClient.prototype.start = function start() {
  ...
  getUserMedia({
    audio: {optional: [{sourceId: ac._mic}]},
    video: false
...
AudioClient.prototype._upsample = function _upsample(audio) {
  ...
  if (ac.rxBuffer.length) {
    v0 = ac.rxBuffer[ac.rxBuffer.length-1]; // get the last item in rxBuffer
  }
  for (i=0;i<samples;i++) {
...
AudioClient.prototype._onRxAudio = function _onRxAudio(event) {
  ...
  if (rxBufLen === 0 || (!playing && rxBufLen < ac.MID_POINT)) {
    ...
    for (i=0; i<FRAME_SIZE; i++) {
      if(ac.talking) {
          pcm = ac.rxBuffer.shift() * 0.25;
      }
      else {
          pcm = ac.rxBuffer.shift();
      }
      flt = parseFloat(pcm / 32768);
      flt = Math.max(flt, -1.0);
      flt = Math.min(flt, 1.0);
      ac.rbuf.push(pcm);
      ac.speakerbuf.push(flt);
    }
    event.outputBuffer.getChannelData(0).set(ac.speakerbuf);
    ...
    var remainderLength = FRAME_SIZE - ac.speakerbuf.length;
    for (i=0; i < remainderLength; i++) {
      ac.rbuf.push(0);
      ac.speakerbuf.push(0.0);
    }
    event.outputBuffer.getChannelData(0).set(ac.speakerbuf);
    ac.playing = false;
  }
