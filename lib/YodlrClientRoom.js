...
YodlrClientRoom.prototype._isMineMessage = function _isMineMessage(data) {
  if (data.room.name === this.name) {
    return true;
  }
  else {
    return false;
...
YodlrClientRoom.prototype._receiveMessage =
  ...
  if(data.type === 'presentation') {
    if(data.payload.type === 'presentationStarted') {
      this._presentationStarted(data);
    }
    if(data.payload.type === 'presentationStopped') {
      this._presentationStopped(data);
    }
    if(data.payload.type === 'presentationPageSet') {
      this._presentationPageSet(data);
    }
  }
};
...
YodlrClientRoom.prototype._presentationStarted = function _presentationStarted(data) {
  if (!this._isMineMessage(data)) {
    return;
  }
  this.presentationActive = true;
  this.presentationFileId = data.payload.fileId;
  this.presentationPage = data.payload.pageNumber;
  this.debug('presentation started');
  this.debug('presentationFile set to ' + data.payload.fileId);
  this.debug('presentationPage set to ' + data.payload.pageNumber);
  this.emit(MSG.presentationStarted, data.payload);
};

YodlrClientRoom.prototype._presentationStopped = function _presentationStopped(data) {
  if (!this._isMineMessage(data)) {
    return;
  }
  this.presentationActive = false;
  this.presentationFileId = null;
  this.presentationPage = null;
  this.debug('presentation stopped');
  this.emit(MSG.presentationStopped, data.payload);
};

YodlrClientRoom.prototype._presentationPageSet = function _presentationPageSet(data) {
  if (!this._isMineMessage(data)) {
    return;
  }
  this.presentationPage = data.payload.pageNumber;
  this.debug('presentationPage set to ' + data.payload.pageNumber);
  this.emit(MSG.presentationPageSet, data.payload);
};
