...
var YodlrClientRoom = module.exports = function YodlrClientRoom(options) {
  ...
  sRoom.captaincabinetClient.on('fileProgress', function onFileProgress(data) {
    sRoom.debug('File upload progress', 
      'fileId:', data.fileId,
  ...
  sRoom.captaincabinetClient.on('fileDeleted', function onFileDeleted(data) {
    sRoom.debug('File deleted',
      'fileName:', data.fileName,
      'fileId:', data.fileId);
    sRoom.sock.emit(MSG.sendFileMessage, {
      user: sRoom.user,
 ...
 * @example
 * sRoom.sendFileMessage('user: myUserName', file: '/myFileName.txt');
 */
 ...
 * @param {string} boxId (optional) The Box ID of the file to delete
 * @example
 * sRoom.sendFileDeleteMessage('user: myUserName', fileName: '/myFileName.txt',
 * fileId: 'xxxx-xxx-xxxx', boxId: 'xxxxxxx');
 */

YodlrClientRoom.prototype.sendDeleteFileMessage =
    function sendDeleteFileMessage(user, fileName, fileId, boxId) {
  var room = this;
  room.debug('Deleting file ', fileName);
  room.captaincabinetClient.FileDelete(room.roomId, fileName, fileId, user, boxId);
 ...
 * @param {string} apiKey
 * @example
 * sRoom.upgradeRoom(email, apiKey);
 */

YodlrClientRoom.prototype.upgradeRoom = function upgradeRoom(email, apiKey) {
  this.sock.emit(MSG.upgradeRoom, {
    roomName: this.name,
    email: email,
    apiKey: apiKey
  });
...
YodlrClientRoom.prototype._joinedRoom = function _joinedRoom(data) {
  ...
  this.presentationFile = data.presentationFileId;
  this.presentationPage = data.presentationPage;
  this.recordingActive = data.recordingActive;
...
YodlrClientRoom.prototype._receiveMessage =
  ...
  // this is an old message, no need to update internal state
  if (data.replayed) {
    return;
  }

  if (data.type === 'presentation') {
    if (data.payload.type === 'presentationStarted') {
      this.debug('Updating presentation data (started)');
      this.presentationActive = true;
      this.presentationPage = data.payload.pageNumber;
      this.presentationFile = data.payload.fileId;
    }
    if (data.payload.type === 'presentationStopped') {
      this.debug('Updating presentation data (stopped)');
      this.presentationActive = false;
      this.presentationPage = 0;
      this.presentationFile = '';
    }
    if (data.payload.type === 'presentationPageSet') {
      this.debug('Updating presentation page');
      this.presentationPage = data.payload.pageNumber;
    }
  }

  if (data.type === 'recording' && data.payload.type === 'recordingStarted') {
    this.recordingActive = true;
  }
  if (data.type === 'recording' && data.payload.type === 'recordingStopped') {
    this.recordingActive = false;
  }

  if (data.type === 'upgrade' && data.payload.type === 'roomUpgraded') {
    this.capabilities = data.payload.capabilities;
    this.roomType = data.payload.roomType;
    this.emit(MSG.upgradedRoom, data);
  }
};
