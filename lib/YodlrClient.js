...
var YodlrClient = module.exports = function YodlrClient(options) {
  ...
  spk.mobileBrowser = 0;
  spk.micSelectSupport = true;
  spk._reconnected = false;

  configPath = 'https://getyodlr.com';
  //configPath = 'https://rooms-staging.getyodlr.com';
  ...
  request(configPath + '/api/config', function configReqResp(err, res, body) {
    if (err || res.status !== 200) {
      debug('Unable to load application config');
      console.error('YodlrClient unable to load application config');
      return spk.emit(MSG.error, new Error('Unable to load app config'));
    }
...
YodlrClient.prototype.connect = function connect() {
  var spk = this;
  if (spk._connecting || spk._reconnected || (spk.sock && spk.sock.connected)) {
  ...
  spk.sock.on('disconnect', function onSocketDisconnect() {
    debug('Disconnected from yakkity');
    spk._disconnected = true;
    spk._connecting = true;
    spk.emit(MSG.disconnected);
    if (spk.audio && spk.audio.client) {
      spk.audio.client.disconnect();
      spk.audio.client = null;
    }
  });
  spk.sock.on('reconnect', function onSocketReconnect() {
    debug('Reconnected to yakkity');
    spk._disconnected = false;
    spk._connecting = false;
    spk._reconnected = true;
    spk.emit(MSG.reconnected);
  });
  spk.sock.on('reconnect_failed', function onReconnectFail() {
    debug('Reconnection failed');
    console.error('Reconnection failed');
  });
  spk.sock.on('reconnecting', function onReconnectingAttempt(attempt) {
    debug('Attempting to reconnect', attempt);
    spk._connecting = true;
  });
...
YodlrClient.prototype._joinRoom = function _joinRoom(spkRoom, data) {
  ...
  this.rooms[data.room].on(MSG.error, this._errorHandler.bind(this));
...
YodlrClient.prototype._joinedRoom = function _joinedRoom(data) {
  ...
  if(this.micSelectSupport && this.audioSources.mics.length === 0) {
    this.rooms[data.room].setMicrophoneEnable(false);
  }
};
...
YodlrClient.prototype._send = function _send(type, data) {
  if (this.sock) { // TODO: improve this to make sure we're connected?
    ...
    debug('Socket not connected, cannot send message', type, data);
...
YodlrClient.prototype._setupAudioClient = function _setupAudioClient(options) {
  ...
  spk.audio.client.on(MSG.disconnected, function onDisconnected() {
    ...
    spk.emit('audioDisconnected');
  });

  spk.audio.client.on(MSG.connected, function onDisconnected() {
    debug('audio client connected');
    spk.emit('audioConnected');
  });

  spk.audio.client.on(MSG.connectionFailure, function onDisconnected() {
    debug('audio client connectionFailure');
    spk.emit('connectionFailure');
  });

  spk.audio.client.on(MSG.connectionTimeout, function onDisconnected() {
    debug('audio client connectionTimeout');
    spk.emit('connectionTimeout');
  });
};
...
YodlrClient.prototype._setSpeakerEnabled = function _setSpeakerEnabled(data) {
  this.audio.client.setSpkrEnable(data.speakerEnabled);
