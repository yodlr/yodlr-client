...
AudioClient.prototype._setupAudioRouterClient = function _setupAudioRouterClient(opts) {
  ...
  ac.arc.on(MSG.ready, function() {
    debug('AudioRouterClient ready');
    ac.connected = true;
    ac.start();
  });
  ...
  ac.arc.on(MSG.connected, function() {
    ...
    ac.emit(MSG.connected);
  });

  ac.arc.on(MSG.connectionFailure, function() {
    debug('AudioRouterClient connectionFailure');
    ac.emit(MSG.connectionFailure);
  });

  ac.arc.on(MSG.connectionTimeout, function() {
    debug('AudioRouterClient connectionTimeout');
    ac.emit(MSG.connectionTimeout);
  });
...
AudioClient.prototype.setMicEnable = function setMicEnable(value) {
  ...
  ac.arc.setMicEnable(value);
};

AudioClient.prototype.setSpkrEnable = function setMicEnable(value) {
  var ac = this;
  ac.ptt = value;
  debug('Setting spkrEnable to ' + value);
  ac.arc.setSpkrEnable(value);
};
